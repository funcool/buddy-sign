...
(ns buddy.sign.jwk-tests
  ...
  (:import (net.i2p.crypto.eddsa EdDSAPrivateKey EdDSAPublicKey)
           ...
           (buddy.com.nimbusds.jose.crypto ECDSA)))

;; Ed25519
;; From RFC8037
(def ed25519-key "{\"kty\":\"OKP\",\"crv\":\"Ed25519\",
...
(deftest ed25519-jwk->jca->jwk
  (let [jwk (jwk/parse-string ed25519-key)
    ...
    (is (= jwk
           (jwk/private-key->jwk private public)))
...
(deftest bi-encoding
  (testing "simple case"
    (let [ba ^bytes (b64/decode "nWGxne_9WmC6hEr0kuwsxERJxWl7MmkZcDusAxyuf2A")
      ...
      (is (= 32 l))
      (is (= (seq ba)
             (seq (BigIntegers/asUnsignedByteArray l bi))))))
...
(deftest rsa-jwk->jca->jwk
  (let [jwk (jwk/parse-string rsa2048-key)
        [public private] (load-pair jwk)]
...
(deftest rsa-load-sign-verify
  (let [jwk (jwk/parse-string rsa2048-key)
        ...
        signature (dsa/sign payload {:alg :rsassa-pkcs15+sha256
                                     :key private})]
    (is (= "cC4hiUPoj9Eetdgtv3hF80EGrhuB__dzERat0XF9g2VtQgr9PJbu3XOiZj5RZmh7AAuHIm4Bh-0Qc_lF5YKt_O8W2Fp5jujGbds9uJdbF9CUAr7t1dnZcAcQjbKBYNX4BAynRFdiuB--f_nZLgrnbyTyWzO75vRK5h6xBArLIARNPvkSjtQBMHlb1L07Qe7K0GarZRmB_eSN9383LcOLn6_dO--xi12jzDwusC-eOkHWEsqtFZESc6BfI7noOPqvhJ1phCnvWh6IeYI2w9QOYEUipUTI8np6LbgGY9Fs98rqVt5AXLIhWkWywlVmtVrBp0igcN_IoypGlUPQGe77Rw"
           (codecs/bytes->str (b64/encode signature true))))
    (is (dsa/verify payload signature {:alg :rsassa-pkcs15+sha256
                                       :key public}))))
...
(deftest rsa-thumbprint
  (let [jwk (jwk/parse-string rsa-pubkey)
        th (jwk/thumbprint jwk)]
...
(deftest ec256-jwk->jca->jwk
  (let [jwk (jwk/parse-string ec256-key)
        [public private] (load-pair jwk)]
...
(deftest ec521-jwk-jca-jwk
  (let [jwk (jwk/parse-string ec521-key)
        [public private] (load-pair jwk)]
...
(deftest ec521-load-sign-verify
  (let [jwk (jwk/parse-string ec521-key)
        ...
        signature (dsa/sign payload {:alg :ecdsa+sha512
                                     :key private})
    ...
    (is (dsa/verify payload signature {:alg :ecdsa+sha512
                                       :key public}))

    (is (dsa/verify payload rfcsig {:alg :ecdsa+sha512
                                    :key public}))))
