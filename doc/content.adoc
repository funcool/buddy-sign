...
(jws/unsign token "key" {:now (time/minus (time/now) (time/seconds 5))})
;; => {:user 1}
----
...
concrete subset of claims: *iat* (issue time), *exp* (expiration time),
*nbf* (not before), *iss* (issuer) and *aud* (audience).
...
;; use timestamp in the past
(jwe/decrypt token "key" {:now (time/minus (time/now) (time/seconds 5))})
;; => {:user 1}
----
